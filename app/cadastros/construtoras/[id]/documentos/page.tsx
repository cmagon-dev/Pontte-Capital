...
export default async function DocumentosConstrutoraPage({ params }: { params: { id: string } }) {
  ...
  // Buscar documentos da construtora
  const documentos = await db.documento.findMany({
    where: { construtoraId: params.id },
    ...
    <div className="p-8">
      <div className="mb-6 flex items-center justify-between">
        <div className="flex items-center gap-4">
          <Link
            href={`/cadastros/construtoras/${params.id}/cadastro`}
          ...
          <div>
            ...
            <p className="text-slate-400">Cofre Digital da Construtora - Validação de Documentos</p>
          </div>
        </div>
      </div>
      ...
      {documentosVencidos.length > 0 && (
        <div className="mb-6 bg-red-950 border border-red-800 rounded-lg p-4">
          ...
          <p className="text-sm text-slate-300">
            {documentosVencidos.length} documento(s) vencido(s) bloqueiam novos desembolsos. Atualize imediatamente.
          </p>
        </div>
      )}

      {/* Lista de Documentos */}
      <DocumentosList documentos={documentos} construtoraId={params.id} />
