...
export default async function NovoUsuarioPage() {
  const [resultadoPerfis, resultadoFundos, resultadoConstrutoras, resultadoObras] = await Promise.all([
    listarPerfis(),
  ...
  const perfis = resultadoPerfis.sucesso
    ? resultadoPerfis.dados!
        .filter((p) => p.ativo)
        .map((p) => ({ id: p.id, nome: p.nome, tipoEscopo: (p as { tipoEscopo?: string }).tipoEscopo ?? 'GLOBAL' }))
    : [];
  ...
  return (
    <div className="p-8 max-w-2xl">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-lg p-6">
        <NovoUsuarioForm perfis={perfis} fundos={fundos} construtoras={construtoras} obras={obras} />
      </div>
    </div>
  );
}
