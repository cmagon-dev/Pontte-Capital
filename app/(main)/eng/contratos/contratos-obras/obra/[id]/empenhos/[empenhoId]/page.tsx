...
export default async function DetalhesEmpenhoPage({ params }: { params: { id: string; empenhoId: string } }) {
  const { id: contratoId, empenhoId } = params;
  ...
  const alertaMinimo = empenho.alertaMinimo ? Number(empenho.alertaMinimo) : 50000;
  const percentualUtilizado = valor > 0 ? ((valor - saldo) / valor) * 100 : 0;
    ...
    <div className="p-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-white font-bold font-mono text-2xl">{formatCurrency(valor)}</p>
                </div>
                <div>
                  <label className="block text-sm text-slate-400 mb-1">Saldo Disponível</label>
                  <p
                    className={`font-bold font-mono text-2xl ${
                      saldo < alertaMinimo ? 'text-red-400' : 'text-green-400'
                    }`}
                  >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              {empenho.observacoes && (
                <div className="col-span-2">
                  ...
                  <p className="text-white text-sm">{empenho.observacoes}</p>
                </div>
              )}
            </div>
          </div>
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-lg p-6">
              ...
              <div className="space-y-3">
                {empenho.documentos.map((doc) => (
                  <div key={doc.id} className="flex items-center justify-between p-4 bg-slate-800 rounded-lg">
                    <div className="flex items-center gap-3">
                      <FileText className="w-6 h-6 text-blue-400" />
                      <div>
                        <p className="text-white font-medium">{doc.nomeArquivo}</p>
                        <p className="text-sm text-slate-400">{doc.tipo}</p>
                      </div>
                    </div>
                    {doc.caminhoArquivo && (
                      <a
          ...
          {/* Documento legado (campo único) */}
          {!empenho.documentos?.length && empenho.nomeArquivo && (
            <div className="bg-slate-900 border border-slate-800 rounded-lg p-6">
              ...
              <div className="flex items-center justify-between p-4 bg-slate-800 rounded-lg">
                <div className="flex items-center gap-3">
                  <FileText className="w-6 h-6 text-blue-400" />
                  <div>
                    <p className="text-white font-medium">{empenho.nomeArquivo}</p>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-lg p-6">
            ...
            <div className="space-y-3">
              <div>
                ...
                <p className="text-sm text-slate-400 mt-1">{empenho.obra.nome}</p>
              </div>
            </div>
          </div>
