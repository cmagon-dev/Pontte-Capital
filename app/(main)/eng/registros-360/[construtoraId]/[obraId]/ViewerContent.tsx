...
import { ArrowLeft, Settings, Map, Camera, Calendar, Layers, X, Loader2, ChevronRight } from 'lucide-react';
import { buscarDadosObra, buscarPlantaBaixa } from '@/app/actions/canteiro-360';
...
export default function ViewerContent({
  ...
  const [loadingFilha, setLoadingFilha] = useState(false);
  ...
  const [historicoNavegacao, setHistoricoNavegacao] = useState<PlantaImplantacao[]>([]);
  ...
  const handleNavegarParaPlanta = async (plantaId: string) => {
    setDrawerAberto(false);
    setLoadingFilha(true);
    setPontoSelecionado(null);
    setFotoSelecionada(null);

    try {
      const resultado = await buscarPlantaBaixa(plantaId);

      if (resultado.success && resultado.data) {
        const plantaFilha: PlantaImplantacao = {
          id: resultado.data.id,
          nome: resultado.data.nome,
          imagemUrl: resultado.data.imagemUrl,
          pontos: resultado.data.pontos.map((p: any) => ({
            id: p.id,
            nome: p.nome,
            x: p.x,
            y: p.y,
          ...
          setores: (resultado.data.setores || []).map((s: any) => ({
            id: s.id,
            nome: s.nome,
            cor: s.cor,
            x: s.x,
        ...
        if (planta) {
          setHistoricoNavegacao((prev) => [...prev, planta]);
        }
        setPlanta(plantaFilha);
      }
    } catch (error) {
      console.error('Erro ao navegar para planta filha:', error);
    } finally {
      setLoadingFilha(false);
    }
  };

  const handleVoltarPlanta = () => {
    if (historicoNavegacao.length === 0) return;
    const historico = [...historicoNavegacao];
    const plantaPai = historico.pop()!;
  ...
  if (loading || loadingFilha) {
    return (
      <div className="h-screen flex items-center justify-center bg-slate-950">
        <div className="text-center">
          <Loader2 className="w-8 h-8 animate-spin text-blue-500 mx-auto mb-4" />
          <p className="text-slate-400">
            {loadingFilha ? 'Carregando planta...' : 'Carregando visualizador...'}
          </p>
        </div>
      </div>
    );
      ...
      <div className="bg-slate-900 border-b border-slate-800 p-4 flex items-center justify-between">
        <div className="flex items-center gap-4">
          {historicoNavegacao.length > 0 ? (
            <button
              onClick={handleVoltarPlanta}
              className="p-2 hover:bg-slate-800 rounded-lg transition-colors"
              title="Voltar para planta anterior"
            >
          ...
          <div>
            ...
            {historicoNavegacao.length > 0 && (
              <div className="flex items-center gap-1 mb-1">
                {historicoNavegacao.map((p, idx) => (
                  <span key={p.id} className="flex items-center gap-1">
                    <button
                      onClick={() => {
                        const novoHistorico = historicoNavegacao.slice(0, idx);
                        const plantaAlvo = historicoNavegacao[idx];
                        setHistoricoNavegacao(novoHistorico);
                        setPlanta(plantaAlvo);
                        setPontoSelecionado(null);
                        setFotoSelecionada(null);
                      }}
