...
interface PlanMedicoesContentProps {
...
export default function PlanMedicoesContent({
  construtorasComObras,
  totalConstrutoras,
  totalObras,
  totalValor,
}: PlanMedicoesContentProps) {
  ...
  return (
    <div className="p-8">
      <div className="mb-6">
        <h1 className="text-3xl font-bold text-white mb-2">Plan.&Medições</h1>
        <p className="text-slate-400">
          Selecione uma construtora para gerenciar o planejamento e medições de suas obras
        </p>
      </div>
        ...
        {filteredConstrutoras.length === 0 ? (
          ...
          filteredConstrutoras.map((grupo) => (
            <div key={grupo.construtoraId} className="bg-slate-900 border border-slate-800 rounded-lg overflow-hidden">
              ...
              <div className="p-4">
                <Link
                  href={`/eng/plan-medicoes/${grupo.construtoraId}`}
