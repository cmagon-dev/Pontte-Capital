...
import { ArrowLeft, Building2, Users, Banknote, Building, PackageOpen, FileText } from 'lucide-react';
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-lg p-4">
          <div className="flex items-center gap-3 mb-2">
            ...
            <span className="text-sm text-slate-400">Custos Indiretos</span>
          </div>
          <p className="text-3xl font-bold text-white">{construtora._count.itensCustoIndireto}</p>
        </div>

        <div className="bg-slate-900 border border-slate-800 rounded-lg p-4">
          <div className="flex items-center gap-3 mb-2">
            <FileText className="w-5 h-5 text-indigo-400" />
            <span className="text-sm text-slate-400">Tipos de Documento</span>
          </div>
          <p className="text-3xl font-bold text-white">{construtora._count.tiposDocumento}</p>
        </div>
      </div>
      ...
      <div className="mb-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <Link
            ...
            className="bg-slate-900 border border-slate-800 hover:border-orange-600 rounded-lg p-6 transition-colors group"
          >
            <PackageOpen className="w-8 h-8 text-orange-400 mb-3 group-hover:scale-110 transition-transform" />
            <h3 className="text-lg font-bold text-white mb-2">Custos Indiretos</h3>
            <p className="text-sm text-slate-400 mb-3">
              Itens de overhead para apropriação em operações
            </p>
            <p className="text-2xl font-bold text-orange-400">{construtora._count.itensCustoIndireto}</p>
          </Link>

          <Link
            href={`/fin/cadastros/${params.id}/tipos-documento`}
            className="bg-slate-900 border border-slate-800 hover:border-indigo-600 rounded-lg p-6 transition-colors group"
          >
            <FileText className="w-8 h-8 text-indigo-400 mb-3 group-hover:scale-110 transition-transform" />
            <h3 className="text-lg font-bold text-white mb-2">Tipos de Documento</h3>
            <p className="text-sm text-slate-400 mb-3">
              Tipos de documento para ordens de pagamento
            </p>
            <p className="text-2xl font-bold text-indigo-400">{construtora._count.tiposDocumento}</p>
          </Link>
        </div>
      </div>
