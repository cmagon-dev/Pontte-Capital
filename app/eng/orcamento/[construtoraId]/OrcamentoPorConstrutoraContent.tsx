...
interface ResumoConstrutora {
  totalObras: number;
  valorTotalPlanilhas: number;
  percentualOrcado: number;
  margemProjetada: number;
}

interface OrcamentoPorConstrutoraContentProps {
...
export default function OrcamentoPorConstrutoraContent({
  construtora,
  obras,
  construtoraId,
  resumo,
}: OrcamentoPorConstrutoraContentProps) {
  ...
  const getPercentualOrcadoColor = (percentual: number) => {
    if (percentual >= 100) return 'bg-green-900 text-green-400';
    if (percentual >= 50) return 'bg-amber-900 text-amber-400';
    return 'bg-red-900 text-red-400';
  };
    ...
    <div className="p-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-lg p-4">
          ...
          <p className="text-2xl font-bold text-white font-mono">{formatCurrency(resumo.valorTotalPlanilhas)}</p>
          <p className="text-xs text-slate-500 mt-1">
            Soma das planilhas contratuais
          </p>
        </div>
        
        <div className="bg-slate-900 border border-slate-800 rounded-lg p-4">
          <p className="text-xs text-slate-400 mb-1">% Orçado</p>
          <p className="text-2xl font-bold text-white">{formatPercent(resumo.percentualOrcado)}</p>
          <p className="text-xs text-slate-500 mt-1">
            Percentual de itens orçados
          </p>
        </div>
        
        <div className="bg-slate-900 border border-slate-800 rounded-lg p-4">
          <p className="text-xs text-slate-400 mb-1">Margem Projetada</p>
          <p className={`text-2xl font-bold font-mono ${getMargemColor(resumo.margemProjetada)}`}>
            {formatPercent(resumo.margemProjetada)}
          </p>
          <p className="text-xs text-slate-500 mt-1">
            Baseado nos itens orçados
          </p>
        </div>
      </div>
          ...
          <table className="table-engineering w-full">
            <thead>
              <tr>
                ...
                <th className="number-cell">% Orçado</th>
                <th>Status</th>
                <th>Ações</th>
              </tr>
            </thead>
              ...
              {filteredObras.length === 0 ? (
                ...
                filteredObras.map((obra) => (
                  <tr key={obra.id} className="hover:bg-slate-800">
                    ...
                    <td className="number-cell">
                      <span className={`px-2 py-1 rounded text-xs font-semibold font-mono ${getPercentualOrcadoColor(obra.percentualOrcado)}`}>
                        {formatPercent(obra.percentualOrcado)}
                      </span>
                    </td>
                    <td>
                    ...
                    <td>
                      <Link
                        href={`/eng/orcamento/${construtoraId}/${obra.id}`}
